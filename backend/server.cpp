...
{
    _logger.log_file(LogLevel::INFO, "Clinet " + _clinet_name + " Has been connected.");
}
...
void Session::handle_read(boost::system::error_code ec){
    ...
    _server.public_hanlde_requet(_parser->release(), _stream);



    if(_parser->keep_alive()){
        do_read();
    }else{
        do_close();
    }
...
void Server::do_accept(){
    _acceptor.async_accept(
        ...
        [this](boost::system::error_code ec, boost::asio::ip::tcp::socket socket){
            if(!ec){
                auto client = std::make_shared<Session>(std::move(socket), *this);
                _clients.emplace(client->get_client_name(), client);
                client->start();
            }else{
